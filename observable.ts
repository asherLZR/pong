...
class Observable<Input> {
  ...
  scan<V>(initialVal:V, fun: (a:V, el:Input) => V): Observable<V> {
    ...
    * @return an Observable which completes amount number of elements are emitted, 
    * undefined if amount provided is <= 0
   */
  take(amount: number): Observable<Input> | undefined {
    function _next(subscriber: Observer<Input>, element: Input, counter: number = 0): void{
      // execute the complete function if the amount to be taken has been met
      if (counter === amount){
        subscriber.complete()
      }else{
        // if it hasn't been met, call the next function on the observer
        subscriber.next(element)
        // return the recursive next function with incremented counter
        return _next(subscriber, element, counter + 1)
      }
    }
    ...
    if (amount <= 0) {
      ...
      return new Observable<Input>((observer: Observer<Input>) => {
        // only call _next() as it will also handle completion
   ...
   /**
    * creates an Observable which only emits the last value of the observed Observable
    * @author asher
    * @return an Observable which only emits the last value of the observed Observable
   */
  last(): Observable<Input> {
    // keep a mutable size 1 array for final value
    const finalValue = [undefined] as [Input?]

    function _next(element: Input): void{
      // update the value of the stored final value
      finalValue[0] = element
    }

    function _complete(subscriber: Observer<Input>){
      // upon completion, subscribe the final value then complete the subscriber
      subscriber.next(finalValue[0]!)
      subscriber.complete()
    }

    return new Observable<Input>(observer => 
      this.subscribe(e => _next(e), () => _complete(observer))
    );
  }

  /**
   * creates a new Observable which performs an effectless function on completion
   * @author asher
   * @param f a function that performs an effectless action
   * @return an Observable which executes f on completion
   */
  finally(f: () => void): Observable<Input> {
    function _complete(subscriber: Observer<Input>){
      // execute the function given upon completion
      f()
      subscriber.complete()
    }

    return new Observable<Input>(observer => 
      this.subscribe(e => observer.next(e), () => _complete(observer))
    );
  }
}
