...
class Observable {
    ...
    take(amount) {
        function _next(subscriber, element, counter = 0) {
            if (counter === amount) {
                subscriber.complete();
            }
            else {
        ...
        if (amount <= 0) {
            ...
            return new Observable((observer) => {
                return this.subscribe(e => _next(observer, e));
            });
        }
    }
    last() {
        ...
        function _complete(subscriber) {
            subscriber.next(finalValue[0]);
            subscriber.complete();
        }
        return new Observable(observer => this.subscribe(e => _next(e), () => _complete(observer)));
    }
    finally(f) {
        function _complete(subscriber) {
            f();
        ...
        return new Observable(observer => this.subscribe(e => observer.next(e), () => _complete(observer)));
    }
}
